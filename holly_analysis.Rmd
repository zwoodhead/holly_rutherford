...
```{r readdata}
setwd("C:/Users/zwoodhead/Dropbox/Project A2/Doppler_Tasks/WG_SG_LG")
...
catcolour<-c('lightblue','grey','pink')
  ...
  thisfile<-'/WSLG_Rawmeans/Rawmeans_001_ListGen.csv'
     plotfile<-read.csv(paste0(mydir,thisfile))
...
par(mfrow=c(3,1),mai = c(.3, .5, 0.5, 0.1), oma=c(1,1,2,1))
# mai specifies bottom, left, top and right margins in inches

for (t in 1:3){ #3 task5
  #set up plot
  thisfile<-paste0(mydir,'/WSLG_Rawmeans/Rawmeans_001_ListGen.csv')
...
catdata<-alldat[,catcol[t]]+2  #convert -1 to 1 to 1:3
  ...
  for (n in 1:nsub){
    thiscol<-thiscol+1
    thisfile<-paste0(mydir,'/WSLG_Rawmeans/Rawmeans_',subnames[n],'_',tasklist[t],'Gen.csv')
...
legend("top", c('Left','Bilateral','Right'), xpd = TRUE, horiz = TRUE, inset = c(0, 
    0), bty = "n", title="Laterality category", lty=1, col=catcolour[c(3,2,1)], cex=1)
...
```{r othersubtract}
listbase <- read.csv(paste0(mydir,'/WSLG_ListBaseline_Data.csv'))
alldat$SG_LG_b.wave<-listbase$LI_SGvLG
poimeans <- read.csv(paste0(mydir,'/WSLG_POImean_Data.csv'))
